...
export class MapView extends graphics.ImgActor {
    constructor(parent, x, y, z, rotation, scale, img, zoneImg, mapData){
	...
	this.mapData.nodes.forEach((e)=>{
	    let newDataArr = new Uint8ClampedArray(imgDataArr.length);
	    for(let i = 0; i < imgDataArr.length; i+=4){
		if(graphics.util.rgbToHex(imgDataArr[i],imgDataArr[i+1],imgDataArr[i+2])===e.color){
